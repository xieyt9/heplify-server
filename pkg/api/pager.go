...
func SetTotalCountHeader(response *restful.Response, count uint64) error {
	response.AddHeader("X-Total-Count", fmt.Sprintf("%d", count))
...
func SetPageLink(baseLink string, response *restful.Response, pager pagination.Pager) (string, error) {
	...
	if response != nil {
		...
		response.AddHeader("X-Total-Count", fmt.Sprintf("%d", pager.ItemTotal()))
	}
